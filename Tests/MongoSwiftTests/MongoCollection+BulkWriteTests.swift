...
final class MongoCollection_BulkWriteTests: MongoSwiftTestCase {
    ...
    func testInserts() throws {
        ...
        expect(try cursor.nextOrError()).to(equal(["_id": 1, "x": 11]))
        expect(try cursor.nextOrError()).to(equal(["_id": result.insertedIds[1]!, "x": 22]))
    ...
    func testUpdates() throws {
        ...
        let cursor = try coll.find()
        expect(try cursor.nextOrError()).to(equal(["_id": 1, "x": 11]))
        expect(try cursor.nextOrError()).to(equal(["_id": 2, "x": 23]))
        expect(try cursor.nextOrError()).to(equal(["_id": 3, "x": 32]))
        expect(try cursor.nextOrError()).to(equal(["_id": 4, "x": 43]))
        expect(try cursor.nextOrError()).to(equal(["_id": 5, "x": 56]))
        expect(try cursor.nextOrError()).to(equal(["_id": result.upsertedIds[3]!, "x": 67]))
        expect(try cursor.nextOrError()).to(beNil())
    }
    ...
    func testDeletes() throws {
        ...
        let cursor = try coll.find()
        expect(try cursor.nextOrError()).to(equal(["_id": 2, "x": 22]))
        expect(try cursor.nextOrError()).to(beNil())
    }
